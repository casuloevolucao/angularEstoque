...
export class CategoriaComponent implements OnInit, AfterViewInit {
  ...
  desativar(categoria){
    Swal.fire({
      title: `Tem certeza que deseja desativar a Categoria ${categoria.nome} ?`,
      type: 'warning',
      ...
      if(result.value){
        this.categoriaS.disableCategoria(this.usuario, categoria).then(()=>{
          Swal.fire({
            title: "Categoria desativada com sucesso!",
            type: 'success'
          })
        })
      }
